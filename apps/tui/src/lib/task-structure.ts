...
export function buildTaskTree(tasks: Record<string, Task>): TaskTreeNode[] {
    const buildNode = (taskKey: string): TaskTreeNode => {
        const childKeys = getDirectChildTaskKeys(tasks, taskKey).sort((a, b) => a.localeCompare(b));
        return {
...
export function findNextParentTaskIndex(taskRows: TaskRow[], currentIndex: number): number {
...
export function findPreviousParentTaskIndex(taskRows: TaskRow[], currentIndex: number): number {
...
export function getDirectChildTaskKeys(tasks: Record<string, Task>, taskKey: string): string[] {
