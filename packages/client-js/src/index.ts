...
export type GetTaskRunLogsResponse = components['schemas']['GetTaskRunLogsResponse'];

export type TaskRunLogsSnapshotMessage = {
    type: 'snapshot';
    runId: string;
    logs: TaskRunLogLine[];
};

export type TaskRunLogsLogMessage = {
    type: 'log';
    log: TaskRunLogLine;
};

export type TaskRunLogsErrorMessage = {
    type: 'error';
    message: string;
};
...
export function createTaskRunnerClient({ port = 7436, host = 'localhost' }: Partial<ClientOptions> = {}) {
    ...
    return {
        ...
        subscribeTaskRun(
            ...
            return openJsonWebSocket<GetTaskRunResponse>(
                buildWsUrl(host, port, `/api/tasks/${encodeURIComponent(runId)}`),
                handlers,
            );
        },
        subscribeTaskLogs(
            ...
            const query = options.includeChildren ? '?includeChildren=true' : '';
            return openJsonWebSocket<TaskRunLogsStreamMessage>(
                buildWsUrl(host, port, `/api/tasks/${encodeURIComponent(runId)}/logs${query}`),
                handlers,
            );
        },
    };
